...
const NewReleases = () => {
    const {NewReleases,setNewReleases} = useContext(NewReleasesContext)
    ...
    const nextPage = (evt)=>{
        ...
        fetch(`http://localhost:4000/movie?type=Movie&sort=-1&slideAmt=${i}`)
        .then(res=>res.json())
        .then((json)=>{
        ...
        <div style={{color:"white",width:"90%",margin:"0 auto"}}>
            ...
            <div className='grid homeScreenPagination' style={{alignItems:'center'}}>
                ...
                <div  className='grid col-5 ml-6' style={{borderBottom:"1px Solid rgba(37,150,190,1)",paddingBottom:"10px",alignItems:"center"}}>
                    {
                        NewReleases.map(featured =>(<NewReleasesList key={featured._id} id={featured._id} img={featured.img}  name={featured.name} rating={featured.rating} trailer={featured.trailer} />))
                    }
                </div>
                <div className='is-size-2' onClick={(evt)=>{
                    nextPage(evt)
